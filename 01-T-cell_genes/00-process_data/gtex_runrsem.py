...
parser.add_argument('--paired_end', type=str.lower, choices=['true', 'false'], default='false', help='Paired-end protocol')
...
with cd(args.output_dir):
    cmd = '/projects/nknoetze_prj/bin/RSEM-1.2.22/rsem-calculate-expression --num-threads '+args.threads+' --fragment-length-max '+args.max_frag_len+' --no-bam-output'
    ...
    if os.path.splitext(args.input_file)[1]=='.bam':
        cmd += ' --bam '+args.input_file+' '+os.path.join(args.rsem_ref_dir,'rsem_reference')+' '+args.prefix
        #+'.rsem'
    else:
        ...
        if args.bowtie_version=='2':
            cmd += ' --bowtie2'
        cmd += ' --bowtie-chunkmbs 128 <(gunzip -c '+fastq1+') <(gunzip -c '+fastq2+') '+os.path.join(args.rsem_ref_dir,'rsem_reference')+' '+args.prefix
        #+'.rsem'
