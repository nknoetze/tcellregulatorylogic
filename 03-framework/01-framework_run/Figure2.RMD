...
zoe_feature_locations_dir <- "/projects/sbrown_prj/220318_DI/data/processed/tcell_ocr_metrics/ZoE_single+allbutmotifs_10k/"
izoe_feature_locations_dir <- "/projects/sbrown_prj/220318_DI/data/processed/tcell_ocr_metrics/iZoE_single+allbutmotifs_10k/"
```
...
process_data <- function(results_file, expression_file,tcell_expressed_tf_file,feature_location_dir){
  ...
  ####### HEATMAPS PROCESSING
  feature_location_files <- list.files(feature_location_dir,pattern='fimo_geneCount.*RAW.featureLocations.tsv$',full.names = TRUE)
  names(feature_location_files) <- basename(feature_location_files) %>% sub(".*\\+(.*?)\\.RAW.*", "\\1", .)
  feature_locations_df <- map_dfr(feature_location_files,fread,.id='region_set') %>% filter(!is.na(score)) %>%
    mutate(feature_region=case_when(region_set=='promoter' ~ paste('p',str_to_upper(feature),sep=''),
                                    region_set=='non-promoter' ~ paste('np',str_to_upper(feature),sep=''),
                                    TRUE ~ paste('a',str_to_upper(feature),sep='')),
           motif_type=ifelse(grepl("PCA_AC",feature),"Novel Motif","Known Motif"))
  
  return(list(expressed_motifs = expressed_motifs_df, significant_motifs=significant_motifs_df,feature_locations=feature_locations_df)) 
}

processed_zoe_files <- process_data(zoe_results,expression_file=gene_expression,tcell_expressed_tf_file = tcell_expressed_tfs,feature_location_dir = zoe_feature_locations_dir)
processed_izoe_files <- process_data(izoe_results,expression_file = gene_expression,tcell_expressed_tf_file = tcell_expressed_tfs,feature_location_dir = izoe_feature_locations_dir)
```
