TITLE = "Oligo Designer - Randomized Gap Sequences"
DESC = "Given a list of candidates, and a number of combos to subset, generate all oligos to be synthesized"
'''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_combos", dest = "NUM_COMBOS", help = "Number of combos to subset.", type = int)
    ...
    parser.add_argument("--gc_content", dest = "GC_CONTENT", help = "Percent GC content for random bases", type = float)
    ...
    statprint("Of {} possible oligos, {} will be created.".format(len(candidates)**args.NUM_CANDIDATES, args.NUM_COMBOS))
    ...
    for cand_set in random.sample(list(itertools.product(candidates.keys(), repeat=args.NUM_CANDIDATES)), args.NUM_COMBOS):
        SUCCEED = False
        while not SUCCEED:

            ## while building these strings, will start with neutral sequence being spaces. Will fill in with actual randomized sequence later.
            ...
            ## if the position is a space or X, fill with random sequence
            ## fill in spaces and Xs (from spacer) with random sequence.
            oligo_array = [x for x in oligo_str]
            for i in range(len(oligo_array)):
                if oligo_array[i] in [" ", "X"]:
                    #oligo_array[i] = random.sample("ACTG", 1)[0]
                    oligo_array[i] = random.choices(["A","T","C","G"], [(100-args.GC_CONTENT)/2,(100-args.GC_CONTENT)/2,args.GC_CONTENT/2,args.GC_CONTENT/2], k=1)[0]
            ...
            if not PASS:
                SUCCEED = False
                statprint("Generated oligo for {} has forbidden sequence {}: {}".format(oligo_name, fs, oligo_str), "WARNING")
            else:
                SUCCEED = True
    ...
    for oligo_name in oligos:
        out.write(">{}.{}\n{}\n".format(oligo_name, "random", oligos[oligo_name]))
    out.close()    
